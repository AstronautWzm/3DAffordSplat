...
class finetune_trainer():
    def __init__(self):
        ...
        self.train_finetune_args = self.train_args['finetune']
        self.train_lora_args = self.train_args['lora']
        self.data_finetune_args = self.data_args['finetune']

        self.ckpt_epoch = self.train_finetune_args['ckpt_epoch']
        self.ckpt_path = self.train_finetune_args['ckpt_path']
        self.max_checkpoints = self.train_finetune_args['max_checkpoints']
        self.is_resume = self.train_finetune_args['is_resume']
        self.gpu_num = self.train_args['gpu_num']
        self.setting = self.data_args['setting']
        self.is_lora = self.train_finetune_args['is_lora']
        self.freeze_models = self.train_finetune_args['freeze_models']


        writer_path = 'runs/affordsplat/finetune/'+self.setting
        ...
        self.logger = logger_init(self.rank,"finetune",self.setting,self.is_resume)
        ...
        if self.freeze_models:
            ...
            for name, module in self.model.named_modules():
                if name in self.freeze_models:
                    ...
                    frozen_modules_name.append(name)
            frozen_str = " | ".join(frozen_modules_name)
            self.logger.info(f"Freeze models: {frozen_str}")
    ...
    def init_dataset(self,start_time):
        ...
        self.dataset_finetune = QAffordSplat_train(pc_num=self.data_finetune_args['pc_num'],
                                           setting=self.data_args['setting'],
                                           is_pretrain=self.data_finetune_args['is_pretrain'],
                                           gs_label_num=self.data_finetune_args['gs_label_num'],
                                           random_seed=self.data_finetune_args['random_seed'],
                                           root_dir=self.data_args['root_dir'])

        finetune_sampler = torch.utils.data.distributed.DistributedSampler(self.dataset_finetune)

        self.dataloader_finetune = data.DataLoader(self.dataset_finetune,
        ...
        start_time = self.log_with_time(start_time,"Finetune dataloader builded up,len(AffordSplat)={}".format(self.dataset_finetune.__len__()))
    ...
    def save_checkpoint(self,epoch, overall_mean_loss):
        if self.rank==0:
            if (epoch + 1) % self.ckpt_epoch == 0:
                ...
                if isinstance(merged_model.mllm.model, PeftModel) and self.is_lora:
                    merged_model.mllm.model = merged_model.mllm.model.merge_and_unload()


                save_dir = './checkpoints/finetune/'+self.setting
                os.makedirs(save_dir, exist_ok=True)

                checkpoint_path = os.path.join(save_dir, f"finetune_"+self.setting+f"_{epoch}.pth")
                ...
                torch.save({
                    ...
                    'optimizer_state_dict': self.finetune_optimizer.state_dict(),
                    'scheduler_state_dict': self.finetune_scheduler.state_dict(),
                    'overall_mean_loss': overall_mean_loss
                }, checkpoint_path)
                self.logger.info(f"Epoch:{epoch} | Checkpoint saved to {checkpoint_path}")


                checkpoints = [f for f in os.listdir(save_dir) if f.startswith("finetune_") and f.endswith(".pth")]
    ...
    def finetune(self):
        ...
        self.finetune_optimizer = torch.optim.Adam(params=self.model.parameters(),
                                          lr=self.train_finetune_args['lr'],
                                          betas=(0.9, 0.999),
                                          eps=1e-8,
                                          weight_decay=self.train_finetune_args['weight_decay'])


        self.total_epoch = self.train_finetune_args['epochs']
        self.start_epoch = 0
        ...
        if self.is_resume:
            ...
            if optimizer_state is not None:
                self.finetune_optimizer.load_state_dict(optimizer_state)
            else:
        ...
        self.finetune_scheduler = torch.optim.lr_scheduler.StepLR(self.finetune_optimizer,
                                                                  step_size=self.train_finetune_args['step_size'],
                                                                  gamma=self.train_finetune_args['gamma'])

        if scheduler_state is not None:
            self.finetune_scheduler.load_state_dict(scheduler_state)

        if self.is_resume:
            self.logger.info(f"Successfully resume training from {self.ckpt_path}")


        self.logger.info(f'————————————————FINETUNE START————————————————')
        ...
        self.logger.info(f"  - Learning Rate: {self.train_finetune_args['lr']}")
        self.logger.info(f"  - Weight Decay: {self.train_finetune_args['weight_decay']}")
        self.logger.info(f"  - Optimizer: Adam")
        self.logger.info(f"  - Scheduler: StepLR(step_size={self.train_finetune_args['step_size']}, gamma={self.train_finetune_args['gamma']})")
        self.logger.info(f"  - World Size: {dist.get_world_size()}")
        self.logger.info(f"  - Every {self.ckpt_epoch} epochs save a checkpoints file")
        self.logger.info(f"  - Training Dataloader Length: {len(self.dataloader_finetune)}")
        ...
        for epoch in iterator:
            ...
            self.dataloader_finetune.sampler.set_epoch(epoch)
            ...
            for i, (obj,                # [Batch size]
                    ...
                    question,           # [Batch size]
                    answer              # [Batch size]
                    ) in enumerate(self.dataloader_finetune):
                ...
                (dynamic_kernels, # [Batch_size, gs_embed_dim, 1]
                 ...
                 ) = self.model(gs_features, padded_gs_datas, mask_batch, pc_mean_all, pc_aff_map_all, question, answer,device=self.device, use_csa=False)


                ce_loss = CE_loss(pred_aff_map,gs_aff_maps,mask_batch)
                dice_loss = Dice_loss(pred_aff_map,gs_aff_maps,mask_batch)
                if "mllm" in self.freeze_models:
                    loss = ce_loss + dice_loss
                else:
                    loss = ce_loss + dice_loss + text_loss


                auc = AUC(pred_aff_map,gs_aff_maps,mask_batch).sum()
                ...
                if self.rank == 0:
                    print(f'Epoch:{epoch}| iteration:{i}|{len(self.dataloader_finetune)} | time consumed:{time.time() - batch_time}')
                    print(f'loss:{loss.item() / len(obj)} | ce_loss:{ce_loss.item() / len(obj)} | dice_loss:{dice_loss.item() / len(obj)} | text_loss:{text_loss.item() / len(obj)} | auc:{auc.item() / len(obj)}')


            overall_mean_loss = loss_sum / data_num
            overall_mean_auc = auc_sum / data_num


            if self.rank == 0:

                self.writer.add_scalar('Train/overall_mean_loss', overall_mean_loss, epoch)
                self.writer.add_scalar('Train/overall_mean_auc', overall_mean_auc, epoch)
            ...
            self.logger.info(
                f'Epoch:{epoch} | Overall_mean_loss:{overall_mean_loss:.4f} | Overall_mean_auc:{overall_mean_auc:.4f} |Time consumed:{time.time() - epoch_time:.2f}s')
        self.log_with_time(finetune_time,f'————————————————FINETUNE END————————————————')
    ...
    def eval(self,epoch):
        ...
        self.logger.info(f"  - World Size: {dist.get_world_size()}")
        self.logger.info(f'  - Evaluation Metrics: Loss, MAE, SIM, KLD, AUC, IOU')
        ...
        with torch.no_grad():
            ...
            for i, (obj,                # [Batch size]
                ...
                gs_aff_maps=torch.tensor(np.array(gs_aff_maps),dtype=self.tensor_type).to(self.device)
                pc_mean_all = torch.randn((self.data_args['batch_size'],self.data_finetune_args['pc_num'],2048, 3),dtype=self.tensor_type).to(self.device)    # 由于模型评估不需要点云数据，故随机初始化点云数据
                pc_aff_map_all = torch.ones((self.data_args['batch_size'],self.data_finetune_args['pc_num'],2048, 3),dtype=self.tensor_type).to(self.device)  # 由于模型评估不需要点云数据，故随机初始化点云数据
            ...
            if dist.get_rank() == 0:
                ...
                if total_metrics['iou'] > self.best_iou:
                    ...
                    checkpoint_path = os.path.join(save_dir, f"finetune_"+self.setting+"_best.pth")
                    if os.path.exists(checkpoint_path):
                        self.logger.warning(f"Checkpoint {checkpoint_path} already exists, overwriting!")
